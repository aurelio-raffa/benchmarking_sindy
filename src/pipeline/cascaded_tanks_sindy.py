...
def main(
    ...
    #     # lambda x: x ** 2,
    #     lambda x: np.sqrt(double_thresh(x)),
    ...
    #     # lambda x: x + "**2",
    #     lambda x: f"sqrt({x})",
    #     # lambda x, y: f'{x} * {y}',  # pairwise product of all terms
    #     # lambda x, y: f'{x} * sqrt({x})',
    #     # lambda x, y: f'sqrt({x}) * {y}',
    # ]
    ...
    # )
    # model.fit(x_train, u=u_train, t=t, x_dot=x_dot_train)
    # model.print()
    #
    # # x_sim = model.simulate(x_train[0, :], t, u=u_train)
    # # plt.plot(t, x_train, label='true')
    ...
    # z_train = sim_hidden_state(t, u_train, tf)

    feature_names = ['xd', 'x', 'z', 'u']
    functions = [
        lambda y: y,  # identity for all terms
    ...
    [k1_opt, k3_opt, z0_opt] = [16.0, 42.0, 0.6]
    ...
    results = pd.DataFrame(
        np.concatenate([t.reshape(-1, 1), rescale_factor * y_out.reshape(-1, 1)], axis=1),
        columns=['time', 'x']
    )
    print(results)
    results.to_csv(os.path.join(root_path, 'outputs/SINDy_bad_library.csv'), index=False)
    ...
    plt.plot(t, x_dot_train, label='est')
    ...
    z_test = sim_hidden_state(t, u_test, tf, k1=k1_opt, k3=k3_opt, z0=z0_opt)
    x_sim = model.simulate(x_test[0, :], t, u=z_test)
    x_sim = jerk(np.sqrt(x_sim).ravel(), dt, params=[tvr_gamma])[1]
    x_sim = np.pad(x_sim, (0, 1), mode='edge').reshape(-1, 1)
    plt.plot(t, x_test, label='true')
    plt.plot(t, x_sim, label='sim')
    plt.legend()
    plt.show()

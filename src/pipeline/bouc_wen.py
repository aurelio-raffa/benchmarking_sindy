...
def zdot_model(nu: float = 1.0, z_term: bool = True, threshold: float = 1.0):
...
def bouc_wen(
    ...
    """Compares a naive SINDy model with a more sophisticated approach on the Bouc-Wen hysteresis benchmark.
    """
    # 01 - Loading the data
    ...
    # TODO: HPO also on this model (esp. threshold)
    m0, _ = xdot_model(include_z=False)
    m0.fit(np.concatenate([x_t, y_t], axis=1), u=u_t, x_dot=np.concatenate([xd_t, x_t], axis=1))

    xy_sindy = m0.simulate(np.array([x_v[0, 0], y_v[0, 0]]), t_t, u=u_v)
    ...
    # TODO: use HPO to tune parameters
    ...
    search_space = {
        'm': hp.uniform('m', 1.0, 10.0),
        'k': hp.uniform('k', 1e4, 1e5),
        'c': hp.uniform('c', 1.0, 100.0),
        'nu': hp.choice(
            'nu_',
            [
                hp.uniform('nu', 0.5, 1.5),
                1.0
            ]
        )
    }
    ...
    m1, m2, m2_coef, x_full, y_full, z_full, z_mod_rmse, z_mod_r2, z, z_est = fit_z_model(
        t_t, y_t, x_t, u_t, xd_t,
        t_v, y_v, x_v, u_v, xd_v,
        alternating_iterations=alternating_iterations,
        tvr_gamma=tvr_gamma,
    ...
    ax1.plot(t_t, z_est, label='z (estimated)')
    ax1.plot(t_t, z_full, label='z (simulated)')
    ax1.plot(t_t, z, label='z (c)')
    ax1.legend()

    ax2.plot(t_t, x_v, label='x', zorder=4)
    ax2.plot(t_t, x_full, label='x (simulated)', zorder=3)
    ax2.plot(t_t, x_sindy, label='x (sindy)', zorder=2)
    ax2.legend()

    ax3.plot(t_t, y_v, label='y', zorder=4)
    ax3.plot(t_t, y_full, label='y (simulated)', zorder=3)
    ax3.plot(t_t, y_sindy, label='y (sindy)', zorder=2)
    ax3.legend()
